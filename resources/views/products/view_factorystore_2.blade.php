{{--  View FactoryStore 2nd Template  --}}
@section('content')
...
<div style="background-size: 100% 100%; background-color: {{ $background_color }}; background-image: url('../images/backend_images/backgrounds/large/{{$background_img}}'); background-repeat: no-repeat;">
	...
	<div class="container">
		...
		<div class="fb-profile"  style="margin-bottom: 40px; margin-top: 0px;" >
            ...
            <div align="center">
            <img style="border-radius: 40%;" onerror="imgError(this);"  class="" src="{{ asset('images/factoryend_images/factory_images/small/'.$factory->factory_image)}} " alt="Profile image example"/>
            </div>
		...
		<div class="tab-content"  >
												...
												<div class="product-image-wrapper" style=" border-color:transparent ">
													<div class="single-products">
														<div  class="productinfo text-center">
															<img style="border-radius: 40%;" src="{{ asset('images/supplierend_images/products/small/'.$product->image) }}" alt="" />
															<h2 style="color: {{ $main_color }}">Rs {{$product->price}}</h2>
															<p style="color: {{ $secondary_color }};"><strong>{{$product->product_name}}</strong></p>
															<a href="{{ url('product/'.$product->id)}}" class="btn btn-default add-to-cart" style="border-radius: 30%;"><i class="fa fa-eye"></i>View</a>
														</div>
													</div>
													<div class="choose text-center" style="border-color: transparent">
																<form name="addtoWishListForm" id="addtoWishListForm" action="{{ url('add-wishlist') }}" method="post">{{ csrf_field() }}
																	...
																	<button type="submit" class="btn btn-default" style="background-color: tomato;color:white; border-radius: 30%;" id="wishlistButton">
			...
			<div class="tab-pane fade" id="care">
				<div class="col-sm-12">
					...
					<h3 style="color: {{ $main_color }};">First Name    :</h3><h4 style="color: {{ $secondary_color }};">{{$factory->name}}   </h4> <br>
					<h3 style="color: {{ $main_color }};">Last Name     :</h3><h4 style="color: {{ $secondary_color }};">{{$factory->last_name}} </h4><br>
					<h3 style="color: {{ $main_color }};">Factory Email   :</h3><h4 style="color: {{ $secondary_color }};">{{$factory->email}}   </h4><br>
					<h3 style="color: {{ $main_color }};">Factory Address :</h3><h4 style="color: {{ $secondary_color }};">{{$factory->address}} </h4><br>
					<h3 style="color: {{ $main_color }};">Phone Number  :</h3><h4 style="color: {{ $secondary_color }};">{{$factory->mobile}}  </h4><br>
					<h3 style="color: {{ $main_color }};">Product Types :</h3><h4 style="color: {{ $secondary_color }};">{{$factory->deals_in}} </h4><br>
					</div>
				</div>
			</div>
			<div class="tab-pane fade" id="branches">
                @foreach ($branches as $branch)
                    <div class="col-sm-12">
                        ...
                        <h3 style="color: {{ $main_color }}; margin-top:-1%">Branch Location     :</h3><h4 style="color: {{ $secondary_color }};">{{$factory->last_name}} </h4><br>
                        <h3 style="color: {{ $main_color }}; margin-top:-1%">Branch Contact Number   :</h3><h4 style="color: {{ $secondary_color }};">{{$factory->email}}   </h4><br>
                        </div>
